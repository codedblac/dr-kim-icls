...
-- 
-- TABLE: blog_posts
--   - Manage blog articles with rich text editing (WYSIWYG editor)
--   - SEO fields for title and description
--   - Publishing controls (draft/published states)
--   - Cover image support with drag-and-drop upload
--   
-- TABLE: articles
--   - Manage educational resources, books, and downloadable files
--   - Free/paid content support with pricing
--   - Amazon affiliate links for book recommendations
--   - File upload and URL storage support
--   
-- TABLE: testimonials
...
--   
-- TABLE: admin_users
--   - Manage admin account access with role-based control
--   - Role-based access control: admin (full access) or editor (content only)
--   - APPROVAL SYSTEM: New accounts must be approved before login
--   - Track who created and approved each account with timestamps
--   - Active/inactive toggle for account management
--   - Password reset functionality with email links
--   - Account editing with role and status changes
--
-- SECURITY FEATURES:
--   - Row-Level Security (RLS) on all tables
--   - Public read access to published content only
--   - Authenticated-only admin access for unpublished content
--   - Admin-only account creation, update, and deletion
--   - Approval requirement: new admins cannot login until approved
--   - Login validation ensures is_approved status before session
--   - Admins can only approve/reject if they are approved themselves
--
-- ADMIN FEATURES:
--   - WYSIWYG blog editor with formatting tools (H1-H4, bold, italic, underline, lists, highlight)
--   - Real-time HTML preview in blog editor
--   - Drag-and-drop image upload to blob storage
--   - Forgot password with email reset link
--   - Account approval workflow for security
--   - Account editing with role and status management
--   - Account deletion with confirmation dialogs
--   - Admin account sidebar navigation
--   - Dashboard with statistics and management overview
--
-- ============================================================================
